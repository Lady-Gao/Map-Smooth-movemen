...
(function () {
    ...
    //跳点
    c.prototype.setTranck=function(idx){
        this.pause()
    ...
    c.prototype.start = function (data,speed) {
         this._opts.playFlag = true
        this._path = data || this.trankpoint
        this._opts.speed = speed ||this._opts.speed
        var f = this,
        e = f._path.length;
        if (f.i && f.i < e - 1) {
            if (!f._fromPause) {
    ...
    }, c.prototype.stop = function (falg) {
        if (falg) {
    ...
    c.prototype.setSpeed = function (val) {
        this.pause()
    ...
    a.object.extend(c.prototype, {
        _addMarker: function (f) {
            ...
            e.setIcon(
                new BMap.Icon(carPng, new BMap.Size(52, 26), {
        ...
        _getDistance: function (f, e) {
            let a = Math.pow(f.x - e.x, 2)
            let b=Math.pow(f.y - e.y, 2)
        ...
        _moveNext: function (e) {
            ...
            if (e < this._path.length - 1) {
                f._Gnum++
                this._opts.showData=f._Gnum
                f._move(f._path[e], f._path[e + 1], f._tween.linear )
            }
        },
        _setInfoWin: function (g,n) {
            if(!n)n=0
            var f = this;
            if (!f._overlay) {
                return
            ...
            if (e != -1) {
                ...
                if (f._Gnum == f._path.length){

                    f._Gnum=0
                    //停止信号
                    this._opts.playFlag && (this._opts.playFlag = false)
                }else{
                    f._overlay.setHtml(f._opts.defaultContent(f._Gnum))
                }
            }
        },
    ...
    a.object.extend(d.prototype, {
        setPosition: function (h, i) {
           ...
           var overlayW = parseInt(this._div.clientWidth || e, 10), overlayH = parseInt(this._div.clientHeight || g, 10);
